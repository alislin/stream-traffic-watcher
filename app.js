...
function updateRecentDailyData(monthlyData) {
    ...
    fs.writeFileSync(DAILY_DATA_FILE_RECENT, JSON.stringify(recentDailyData, null, 2));
}
...
function displayData(data) {
    ...
    console.log(`剩余流量: ${remaining} (${Math.round((data.bw_counter_b / data.monthly_bw_limit_b) * 100)}%)`);
    console.log(`剩余天数: ${remainingDays}`);
    console.log(`今日已用: ${todayUsage}`);
    console.log('-------------------- 最近 30 天使用量 --------------------');
    Object.keys(recentDailyData).sort().forEach(date => {
